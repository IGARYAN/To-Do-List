...
<p align="center">
  <img src="static/screenshots/task-modal.png" alt="Создание задачи" width="30%" />
  <img src="static/screenshots/theme-screen.png" alt="Тёмная тема" width="30%" />
  <img src="static/screenshots/settings-screen.png" alt="Настройки" width="30%" />
</p>
...
- ⚙️ **Гибкие настройки** — автозапуск при старте системы, всегда поверх всех окон, сохранять и восстанавливать состояние окна с учетом масштаба экрана (DPI)
...
## 🖥️ Восстановление состояния окна

Приложение автоматически сохраняет и восстанавливает состояние окна (позицию, размеры, максимизацию) при закрытии и запуске. Для корректного восстановления учитываются:

- **Разрешение экрана** — если разрешение изменилось, окно будет размещено по центру
- **Масштаб экрана (DPI)** — при изменении масштаба размеры окна автоматически корректируются пропорционально
- **Монитор** — если окно было на одном мониторе, а теперь на другом, состояние не восстанавливается

При изменении масштаба экрана приложение рассчитывает новый коэффициент масштабирования и корректирует размеры окна, чтобы сохранить визуальную пропорцию. Например, если сохранённый scaleFactor был 1.25, а текущий 1.5, размеры окна будут увеличены на 20% (1.5 / 1.25 = 1.2).
