...
**To-Do List** — это десктопное приложение для управления задачами, созданное на базе современного стека **Tauri + SvelteKit + TypeScript**. Оно позволяет удобно вести список дел, устанавливать напоминания, создавать повторяющиеся шаблоны задач и защищать данные паролем с помощью шифрования.
...
- 🎨 **Настраиваемая цветовая схема** — меняйте цвета интерфейса под свои предпочтения
- 🌙 **Тёмная тема** — поддержка светлой и тёмной темы оформления
- 📊 **Статистика** — отслеживайте свою продуктивность с помощью встроенной статистики
- ⚙️ **Гибкие настройки** — автозапуск при старте системы, всегда поверх окон, настройка уведомлений
- 🔄 **Автоматическое обновление версий** — скрипт для обновления версии приложения
...
- [Rust](https://www.rust-lang.org/) (установленный через `rustup`)
