<script lang="ts">
    ...
    function toggleActive(template: TypesRepeatTemplate) {
        ...
        toastStore.add({
            title: template.isActive ? "Шаблон приостановлен" : "Шаблон активирован",
...
{:else}
    <div class="space-y-2">
        {#each taskStore.templates as template (template.id)}
            <Card.Root class={`px-2 py-2 transition-all duration-300 ${!template.isActive ? "opacity-50" : ""}`}>
                <div class="flex items-center gap-3">
                    <!-- Цветная полоса -->
                    <div
                        class="w-2 h-10 rounded flex-shrink-0"
                        style="background-color: {ColorMap[template.color ?? 'red']}"
                    ></div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            ...
                            {#if !template.isActive}
                                <Badge variant="outline" class="text-xs">Пауза</Badge>
                            {/if}
                        </div>
                    ...
                    <DropdownMenu.Root>
                        ...
                        <DropdownMenu.Content align="end">
                            <!-- Редактировать -->
                            <DropdownMenu.Item onclick={() => appStateStore.openTemplateModal(template)}>
                                <Pencil class="h-4 w-4" />
                                <span>Редактировать</span>
                            </DropdownMenu.Item>

                            <!-- Пауза / Возобновить -->
                            <DropdownMenu.Item onclick={() => toggleActive(template)}>
                                {#if template.isActive}
                            ...
                            <!-- Удалить -->
                            <DropdownMenu.Item onclick={() => confirmDelete(template)}>
...
<Dialog.Root open={isDeleteDialogOpen} onOpenChange={closeDeleteDialog}>
    <Dialog.Content class="sm:max-w-md">
        ...
        <p class="text-sm text-muted-foreground">
            ...
            <span class="font-medium text-foreground">"{templateToDelete?.title}"</span>?
        </p>
        <div class="flex flex-col gap-2 pt-2">
            <Button variant="outline" onclick={deleteTemplateOnly} class="w-full">
                Удалить только шаблон
            </Button>
            <Button variant="destructive" onclick={deleteTemplateWithTasks} class="w-full">
                Удалить шаблон и будущие задачи
            </Button>
            <Button variant="ghost" onclick={closeDeleteDialog} class="w-full">
                Отмена
            </Button>
        </div>
    </Dialog.Content>
